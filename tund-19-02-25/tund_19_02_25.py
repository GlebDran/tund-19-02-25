import numpy as np #diapazon znachenij [min, max]
import matplotlib.pyplot as plt

def vaal ():
    x1=np.arange(0, 10,1) # pervqe 2 chisla diapazon, poslednee shag
    y1=(2/27)*x1**2-3
    ...
    plt.figure(facecolor="lightblue") #PLT = MATPLOTLIB (cvet fona)
    plt.title("Рыбка кииииит") #nazvanie figurq
    plt.ylabel("Y")
    plt.xlabel("X")
    plt.grid(True)
    ax=plt.axes()
    ...
    for i in range (1, 11):
        plt.plot(eval(f"x{i}"), eval(f"y{i}"),colors[i-1]+"-*") #eval(f"{colors[i-1]}-*"))
    plt.show()
